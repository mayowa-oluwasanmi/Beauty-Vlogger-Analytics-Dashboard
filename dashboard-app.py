...
#Set page config
st.set_page_config(page_title="Beauty Vlogger Dashboard", layout="wide")

#Functions
@st.cache_data
#Load in csv data
def load_data():
    data = pd.read_csv("beauty_blogger_youtube_data.csv")
...
def format_with_commas(number): #shows number with a comma
    return f"{number:,}"
...
st.logo(image="images/5237351.jpg",
    icon_image="images/icon.ico")

with st.sidebar:
    st.title("Beauty Vlogger Analytics")
